...
impl UpdateObjectExt for UpdateObject {
    fn try_parse_date_time(&self) -> Result<DateTimeConvert, ParseError> {
        ...
        let date_ticks = self.payload[0..8]
            ...
            .map_err(|_| ParseError::InvalidPayload { expected: 8, actual: self.payload[0..8].len() })?;
        let sync_time_seconds = self.payload[8..16]
            .try_into()
            .map(f64::from_le_bytes)
            .map_err(|_| ParseError::InvalidPayload { expected: 8, actual: self.payload[8..16].len() })?;
        ...
        let date_time = DateTime::from_timestamp(utc_seconds, unix_nanos as u32)
