...
pub fn get_credential_with_simple_prompt(client: &ApiClient) -> Result<Credential> {
    ...
    let res_version = res_version.unwrap_or(api::BASE_CLIENT_VERSION.to_string());
    let client_version = client_version.unwrap_or(api::BASE_CLIENT_VERSION.to_string());
